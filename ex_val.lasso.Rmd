...
settings <- list(
  ...
  master_formula=Y ~ age + miloc + pmi + kill + pmin(sysbp,100) + lsp(pulse,50)
)
...
```{r}
  ...
  mm <- model.matrix(settings$master_formula, data=dev_data)
  tmp <- cv.glmnet(x=mm, y=dev_data$Y, family="binomial")
  model <- glmnet(x=mm, y=dev_data$Y, family="binomial", lambda=tmp$lambda.min)
  pi <- predict(model, type="response", newx=model.matrix(settings$master_formula,val_data))
...
```{R}
  ...
  for(i_sim in 1:settings$n_sim_mb)
  {
    ...
    mm <- model.matrix(settings$master_formula, data=val_data)
    tmp <- cv.glmnet(x=mm, y=val_data$Y, family="binomial",weights = val_data$w_y)
    pop_model <- glmnet(x=mm, y=val_data$Y, family="binomial", lambda=tmp$lambda.min, weights = as.vector(val_data$w_y))
    
    p <- predict(pop_model, type="response", newx = mm)
