...
# #And now uncertainty around it
  bs_NBh_model <- bs_NBh_all <- matrix(0, nrow=settings$n_sim_bs, ncol=length(zs))
...
###  Asymptotic approach
```{R}
  res_as <- voi_ex_glm(model,val_data,method='asymptotic',Bayesian_bootstrap = F,zs = zs)
...
```{r}
max_y <- max(c(res_mb$EVPIv[zs<=settings$max_x_evpi_plot],res_bb$EVPIv[zs<=settings$max_x_evpi_plot],res_ob$EVPIv[zs<=settings$max_x_evpi_plot]))
