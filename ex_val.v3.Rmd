...
```{R}
NBh_model[which(zs %in% settings$zs)]
NBh_all[which(zs %in% settings$zs)]
```

Incremental NB of the model at these thresholds
```{R}
NBh_model[which(zs %in% settings$zs)] - pmax(NBh_all[which(zs %in% settings$zs)],0)
```
