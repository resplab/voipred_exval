...
perturbed_scenario <- function(sample_size,  true_model = c(-1.55,0.77), cintercept=NA, cslope=NA, max_n_inner_sim=1000, zs=0.1, seed=NULL)
{
  ...
  res <- voi_ex_glm(model=NULL, val_data=sample, method = "asymptotic", zs = zs)
...
perturbed_scenario_all <- function(sample_size,  true_model = c(-1.55,0.77), cintercepts=NA, 
                                   cslope=NA, max_n_inner_sim=1000, zs=0.1, seed=NULL)
{
  #cat("Doing ", "Sample_size:",sample_size, "cintercept:", cintercept, "cslope:",  cslope,"\n")
  # if(!is.null(seed))  set.seed(seed)
  ...
  out <- NULL
  for (i in 1:length(cintercepts)){
    
    cintercept <- cintercepts[i]
  ...
  tmp_out <- c(EVPIv=res[,'EVPIv'], NB_model=res2[,'NB_model'], NB_all=res2[,'NB_all'],  auc=auc, prev=prev, check_intercept=check[1], check_slope=check[2], check_mean=check_mean)
  out <- rbind(out,c(sample_size, cintercept, NA, tmp_out))
                                 ...
                                 seed=NULL)
{
  ...
  for(sample_size in sample_sizes)
  {
    if(length(cintercepts)>0)
    {
      ...
      res <- perturbed_scenario_all(sample_size, cintercepts = cintercepts, zs = zs, seed=seed)
      out <- rbind(out,res)
...
saveRDS(look,"perturbation_sim_V7_HL_same_sample.rds")
res <- read_rds("perturbation_sim_V7_HL_same_sample.rds")
res2 <-read_rds("perturbation_sim_V7_HL.rds")
