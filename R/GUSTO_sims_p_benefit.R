...
sim_by_size <- function(n_sim=1000, sample_sizes=c(250, 500, 1000, 2000, 4000, 8000, Inf),zs=c(0.01,0.02,0.05,0.1))
{
  ...
  for(s in 1:length(sample_sizes))
  {
    ...
    for(i in 1:n_sim)
    {
      ...
      out[index,'method'] <- "BB"; out[index,'sample_size']<-sample_size; out[index,c('val1','val2','val3','val4')] <- tmp$p_useful
      index <- index+1
      
      tmp <- voi_ex_glm(model, this_data, method="bootstrap", Bayesian_bootstrap = F, zs = zs)
      if(is.null(tmp)) bad <- T
      out[index,'method'] <- "OB"; out[index,'sample_size']<-sample_size; out[index,c('val1','val2','val3','val4')] <- tmp$p_useful
      index <- index+1
